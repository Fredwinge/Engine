...
CTexture::CTexture(CRenderer* pRenderer, const wchar_t* fileName)
{
	...
	if (CImage::MakeFromFile(fileName, &image) == CImage::LOAD_SUCCESSFUL)
	{
		...
		GFX_ASSERT_INFO(pRenderer->GetDevice()->CreateTexture2D(&textureDesc, &sd, &m_pTexture2D));
		...
		GFX_ASSERT_INFO(DirectX::CreateShaderResourceView(pRenderer->GetDevice(), image->GetScratch()->GetImage(0, 0, 0), 1u, image->GetScratch()->GetMetadata(), &m_pTextureView));
	}
	else
	{
		//Create texture Resource
		...
		GFX_ASSERT_INFO(pRenderer->GetDevice()->CreateShaderResourceView(m_pTexture2D.Get(), &srvDesc, &m_pTextureView));
	}
...
void CTexture::Bind(CRenderer* pRenderer) noexcept
{
	pRenderer->GetDeviceContext()->PSSetShaderResources(0u, 1u, m_pTextureView.GetAddressOf());
}